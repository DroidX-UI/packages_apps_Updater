...
updates_dir=/data/droidx_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have DROIDX-VERSION-DATE-TYPE-* as format"
...
# Assume DROIDX-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.droidx.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.droidx.updater/databases/updates.db" \
